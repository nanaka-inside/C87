...
OLTP のユースケースにおいてスケールさせる必要が生じたことで、AmazonやGoogleはそれぞれKVSを構築しました。学術的には、分析用途を完全に捨てたH-Storeが提案されVoltDBと呼ばれるベンチャーが設立されています。それとは別に、SSDを始めとするハードウェアの進歩により、既存のRDBMSの処理スループットが劇的に増加したり、Snapshot Isolation
